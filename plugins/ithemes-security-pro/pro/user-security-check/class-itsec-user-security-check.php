...
class ITSEC_User_Security_Check {
	...
	public function send_2fa_email_reminder() {
		...
		$merge_tags = array();
		$merge_tags['{{title}}']        = sprintf( __( 'Two Factor Authentication Reminder for <a href="%1$s" target="_blank" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%%;-webkit-text-size-adjust: 100%%;color: #FFFFFF;font-weight: bold;text-decoration: underline;">%2$s</a>', 'it-l10n-ithemes-security-pro' ), get_site_url(), $blogname );
	...
	protected function get_password_score( $password, $user ) {
		...
		if ( ! class_exists( 'ITSEC_Zxcvbn' ) ) {
			require_once( 'lib/itsec-zxcvbn-php/zxcvbn.php' );
		}
		$zxcvbn = new ITSEC_Zxcvbn();
		...
		$results = $zxcvbn->test_password( $password, $penalty_strings );
	...
	public function check_inactive_accounts() {
		...
		$merge_tags = array();
		$merge_tags['{{title}}']               = sprintf( __( 'Inactive User Warning for <a href="%1$s" target="_blank" style="mso-line-height-rule: exactly;-ms-text-size-adjust: 100%%;-webkit-text-size-adjust: 100%%;color: #FFFFFF;font-weight: bold;text-decoration: underline;">%2$s</a>', 'it-l10n-ithemes-security-pro' ), get_site_url(), $blogname );
